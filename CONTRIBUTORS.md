...
PRAGMA was designed and built by a five-person team at Canara Bank for the Suraksha Cyber Hackathon 2.0.
...
- **Project scaffold and architecture** — Designed the initial repository structure, module boundaries, and the full 5-person ownership model (`feat: M0 — complete project scaffold for 5-person team`)
...
- **Final enterprise hardening** — Removed all hackathon branding, standardised design tokens, eliminated duplicate audit events, verified production build cleanliness
...
- **Core frontend scaffold** — Built the initial React dashboard layout, MAPs view, approval panel, event log, and upload flow (`feat(frontend): dashboard layout, MAPs, approvals, event log, upload`)
...
- **Design refinements** — Bank-grade premium pass: navy sidebar rail, IBM Plex type system introduction, mono data ledger styling, badge system (`design(frontend): bank-grade premium pass`)
- **Dashboard components** — Shared component library including `LifecycleStrip`, `ResolutionFunnel`, `UpcomingDeadlines`
- **Approvals panel** — Async loading, confirmation state management, and status transition UI (`feat(approvals): async loading + confirmation states`)
- **Register-of-record identity** — Serif masthead, lifecycle hero component, brass design system foundations

---

## Diyasha Nag
**GitHub:** [@Diyaasha](https://github.com/Diyaasha) / Diyasha Nag
**Role:** Backend Engineer

**Contributions:**

- **Backend workflow endpoints** — Implemented the core FastAPI endpoint layer for the MAP and approval workflow
- **Approval workflow** — Complete approval status transitions with validation, status history, and audit event generation (`feat: complete approval workflow with status transitions and audit events`)
...
- **Database layer** — Complete PostgreSQL schema design, SQLAlchemy ORM models, and initial Alembic migration setup (`Feature: Complete PostgreSQL persistence and database integration`)
...
- **Test stabilisation** — Fixed test suite compatibility for Python 3.13, patched endpoint mocks, upgraded SQLAlchemy for compatibility (`Fix remaining test suite errors, patch endpoints mocks, and upgrade SQLAlchemy for Python 3.13 compat`)
- **UUID and source field fixes** — Resolved UUID ID handling and source field consistency across the data pipeline

---

## Contribution Summary by Commit Count

| Contributor | Commits | Primary Area |
|---|---|---|
| Anoushka Nag | 20 | Architecture · AI · Frontend · Product |
| Ashwin Yadav | 12 | Frontend · Dashboard · UX |
| Anuja Chakraborty | 5 | Data pipeline · Tests |
| Diyasha Nag | 3 | Backend APIs · Approval workflow |
| Diptanshu Vishwa | 2 | Database layer |

_Commit counts derived from `git shortlog -sn --all` as of the final submission commit._
