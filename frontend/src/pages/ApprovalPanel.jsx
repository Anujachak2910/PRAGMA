...
function ConfirmModal({ decision, mapAction, onConfirm, onCancel, busy }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm animate-fadeIn">
      <div className="mx-4 w-full max-w-md rounded-2xl border border-line bg-white p-6 shadow-xl">
        {/* Icon */}
        ...
        {/* Body */}
        <p className="mt-2 text-center text-[13px] leading-relaxed text-gray-600">
        ...
        {/* Action preview */}
        <div className="mt-4 rounded-lg border border-line bg-paper/60 px-3 py-2">
          <p className="line-clamp-2 text-[12px] italic text-gray-600">"{mapAction}"</p>
        </div>
        ...
        <div className="mt-5 flex gap-3">
          <button
            onClick={onCancel}
            disabled={busy}
            className="flex-1 rounded-lg border border-line bg-white py-2.5 text-sm font-medium text-gray-600 transition-colors hover:bg-paper disabled:opacity-50"
          >
            Cancel
          </button>
          <button
...
/* ── Circular detail expandable ── */
function CircularInfo({ circular }) {
  const [open, setOpen] = useState(false)
  if (!circular) return null
  return (
    <div className="rounded-lg border border-brass/20 bg-brass-soft/40">
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center justify-between px-3 py-2 text-[12px] text-brass-deep hover:bg-brass-soft/60"
      >
      ...
      {open && (
        <div className="border-t border-brass/20 px-3 py-2.5 animate-fadeIn">
          <p className="font-semibold text-[13px] text-ink">{circular.title}</p>
...
function ApprovalCard({ map, circular, onResolved }) {
    ...
    return (
      <div
        className={`flex items-center justify-center rounded-xl border p-8 animate-fadeIn ${
          approved ? 'border-success-200 bg-success-50' : 'border-danger-200 bg-danger-50'
        }`}
        style={{ minHeight: 220 }}
      >
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className={`font-serif text-lg font-semibold ${
            approved ? 'text-success-700' : 'text-danger-700'
          }`}>
  ...
  return (
    <>
      ...
      <div className="rounded-xl border border-line bg-white p-6 flex flex-col gap-4">
        ...
        {/* Action text */}
        <div>
          <p className="text-[14px] leading-relaxed text-ink">{map.action}</p>
          {map.source_clause && (
        ...
        <div className="flex flex-wrap gap-3 border-t border-line pt-3">
          ...
          <div className="flex items-center gap-1 rounded border border-violet-200 bg-violet-50 px-1.5 py-0.5">
            <Sparkles size={10} className="text-violet-500" />
            <span className="font-mono text-[9px] font-semibold uppercase tracking-wide text-violet-700">
        ...
        <textarea
          ...
          rows={3}
          className="resize-none rounded-lg border border-line bg-paper/40 px-3 py-2 text-sm text-ink placeholder:text-gray-400 focus:border-primary-400 focus:outline-none focus:ring-1 focus:ring-primary-400 disabled:opacity-60"
        />
    ...
    <div className="space-y-5">
      {pending.length === 0 ? (
        <div className="rounded-xl border border-line bg-white p-12 text-center">
          <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full border-2 border-success-200 bg-success-50">
            <Check size={24} className="text-success" strokeWidth={2.5} />
          </div>
          <p className="font-serif text-xl font-semibold text-ink">Review queue is clear</p>
