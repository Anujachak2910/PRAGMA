...
const RISK_BADGE = {
  Critical: 'bg-danger-50 dark:bg-red-900/30 border-danger-200 dark:border-red-800 text-danger-700 dark:text-red-400',
  High:     'bg-warning-50 dark:bg-amber-900/30 border-warning-200 dark:border-amber-800 text-warning-700 dark:text-amber-400',
  Medium:   'bg-brass-soft dark:bg-brass/10 border-brass/40 dark:border-brass/30 text-brass-deep dark:text-brass',
  Low:      'bg-success-50 dark:bg-green-900/30 border-success-200 dark:border-green-800 text-success-700 dark:text-green-400',
}
  ...
  if (!sections.length) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-sm text-gray-400 dark:text-gray-600">No content available for this circular.</p>
      </div>
    )
  }
            ...
            className={`rounded-lg transition-all duration-300 ${
              s.heading
                ? `border p-4 ${isHighlighted
                    ...
                    : 'border-line dark:border-gray-800 bg-paper/20 dark:bg-gray-900/30'
                  }`
            ...
            {s.heading && (
              <div className="mb-2 flex items-start gap-2">
                ...
                <p className={`font-mono text-[11px] font-bold uppercase tracking-wide ${
                  isHighlighted ? 'text-brass-deep dark:text-brass' : 'text-gray-500 dark:text-gray-600'
                }`}>
                  {s.heading}
                </p>
              </div>
...
export default function ExtractionReview() {
  ...
  // Pick the first circular once loaded
  useEffect(() => {
    ...
    <div className="flex flex-col gap-4 h-[calc(100vh-9rem)]">
      ...
      <div className="flex flex-wrap items-center gap-3 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-gray-500 dark:text-gray-500">
            AI Extraction Review
          </p>
        </div>
        <div className="h-4 w-px bg-line dark:bg-gray-800" />
        {/* Circular selector */}
        <div className="relative">
          <select
            value={selectedCircularId}
            onChange={(e) => { setSelectedCircularId(e.target.value); setSelectedMapId(null) }}
            className="appearance-none rounded-lg border border-line dark:border-gray-700 bg-white dark:bg-gray-900 py-1.5 pl-3 pr-8 text-sm font-medium text-ink dark:text-gray-200 focus:border-primary-400 focus:outline-none focus:ring-1 focus:ring-primary-400 min-w-[280px]"
          >
            {circulars.map((c) => (
      ...
      <div className="flex gap-5 min-h-0 flex-1">
        {/* LEFT: Circular text */}
        <div className="flex-1 min-w-0 flex flex-col rounded-xl border border-line dark:border-gray-800 bg-white dark:bg-gray-950 overflow-hidden">
          <div className="flex items-center gap-2 border-b border-line dark:border-gray-800 bg-paper/40 dark:bg-gray-900/40 px-5 py-3 flex-shrink-0">
        ...
        {/* RIGHT: MAPs panel */}
        <div className="w-96 flex-shrink-0 flex flex-col rounded-xl border border-line dark:border-gray-800 bg-white dark:bg-gray-950 overflow-hidden">
          <div className="flex items-center gap-2 border-b border-line dark:border-gray-800 bg-paper/40 dark:bg-gray-900/40 px-5 py-3 flex-shrink-0">
            <Brain size={13} className="text-violet-500" />
            <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-gray-500 dark:text-gray-500">
              Extracted MAPs
            </p>
            <span className="ml-auto rounded-full bg-ink/10 dark:bg-gray-800 px-2 py-0.5 font-mono text-[9px] text-gray-600 dark:text-gray-500">
              {circlMaps.length}
            </span>
          </div>

          <div className="flex-1 overflow-y-auto divide-y divide-line dark:divide-gray-800">
            {loading ? (
              <div className="p-4">
              ...
              <div className="flex flex-col items-center gap-2 p-8 text-center">
                <Layers size={24} className="text-gray-200 dark:text-gray-800" />
                  ...
                  <button
                    ...
                    className={`w-full text-left px-5 py-4 transition-colors ${
                      isSelected
                        ...
                        : 'hover:bg-paper/60 dark:hover:bg-gray-900/60 border-l-2 border-l-transparent'
                    }`}
                  >
                    {/* Source clause badge */}
                    {m.source_clause && (
                      <p className={`mb-1.5 font-mono text-[9px] font-semibold uppercase tracking-wide ${
                        isSelected ? 'text-brass-deep dark:text-brass' : 'text-gray-400 dark:text-gray-600'
                      }`}>
                        {m.source_clause}
                      </p>
                    )}
                    ...
                    <p className={`text-[12px] leading-snug line-clamp-3 ${
                      isSelected ? 'text-ink dark:text-gray-100 font-medium' : 'text-gray-700 dark:text-gray-300'
                    }`}>
                      {m.action}
                    </p>
                    ...
                    {isSelected && m.reasoning && (
                      <div className="mt-3 rounded-md border border-violet-200 dark:border-violet-800 bg-violet-50/50 dark:bg-violet-900/20 px-3 py-2">
          ...
          {/* Footer */}
          <div className="border-t border-line dark:border-gray-800 px-5 py-2.5 flex-shrink-0">
