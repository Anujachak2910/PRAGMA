...
const EVIDENCE_ICON = {
  ...
  screenshot:  { icon: ExternalLink, cls: 'text-gray-500 dark:text-gray-400',   bg: 'bg-gray-50 dark:bg-gray-800' },
  certificate: { icon: BookOpen,     cls: 'text-brass dark:text-brass',          bg: 'bg-brass-soft dark:bg-brass/10' },
  log:         { icon: FileText,     cls: 'text-gray-500 dark:text-gray-400',   bg: 'bg-gray-50 dark:bg-gray-800' },
  ...
  approval:    { icon: BookOpen,     cls: 'text-brass dark:text-brass',          bg: 'bg-brass-soft dark:bg-brass/10' },
...
function Section({ icon: Icon, title, children, accent = false }) {
  return (
    <div className={`rounded-lg border p-4 ${
      accent
        ? 'border-primary-200 dark:border-primary-800 bg-primary-50/40 dark:bg-primary-900/20'
        : 'border-line dark:border-gray-800 bg-paper/30 dark:bg-gray-900/40'
    }`}>
...
export default function TraceabilityDrawer({ map, circular, onClose }) {
  ...
  return (
    <>
      ...
      {/* Drawer panel */}
      <aside className="fixed right-0 top-0 z-50 flex h-full w-[480px] max-w-[95vw] flex-col bg-white dark:bg-gray-950 shadow-2xl animate-slideIn overflow-hidden">
        {/* Header */}
        <div className="flex items-start justify-between gap-4 border-b border-line dark:border-gray-800 px-6 py-4 flex-shrink-0">
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <Brain size={14} className="text-violet-500 flex-shrink-0" />
              <p className="font-mono text-[9px] uppercase tracking-[0.2em] text-gray-500 dark:text-gray-500">
                AI Traceability
              </p>
            </div>
            <h3 className="font-serif text-base font-semibold leading-snug text-ink dark:text-gray-100">
          ...
          <button
            onClick={onClose}
            className="mt-0.5 flex-shrink-0 rounded-md p-1.5 text-gray-400 dark:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
          ...
          {map.clause_text && (
            <Section icon={BookOpen} title="Original Regulatory Clause" accent>
              <blockquote className="border-l-2 border-primary-300 dark:border-primary-700 pl-3">
                <p className="text-[12.5px] leading-relaxed text-gray-700 dark:text-gray-300 italic">
                  "{map.clause_text}"
                </p>
              </blockquote>
              {circular && (
              ...
              <div className="space-y-2">
                {map.evidence_docs.map((doc, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="flex items-center gap-3 rounded-md border border-line dark:border-gray-800 px-3 py-2"
                    >
                      <div className={`flex h-7 w-7 flex-shrink-0 items-center justify-center rounded ${cfg.bg}`}>
                        <Icon size={12} className={cfg.cls} />
                      </div>
                      <div className="min-w-0">
                        <p className="truncate text-[12px] font-medium text-ink dark:text-gray-200">
                          {doc.name}
                        </p>
                        <p className="font-mono text-[9px] uppercase tracking-wide text-gray-400 dark:text-gray-600">
                          {doc.type}
                        </p>
                      </div>
                    </div>
        ...
        {/* Footer */}
        <div className="border-t border-line dark:border-gray-800 px-6 py-3 flex-shrink-0">
